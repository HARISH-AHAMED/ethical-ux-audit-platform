...
import { ShieldCheck, History, Scale, Menu, X, LayoutDashboard, BookOpen } from 'lucide-react'
...
function App() {
  ...
  return (
    <div className="min-h-screen text-slate-100 relative selection:bg-primary-500/30" style={{ backgroundColor: '#020617' }}>
      {/* Layer 1: Premium Background Glow */}
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-primary-900/20 via-slate-950/80 to-slate-950 pointer-events-none" style={{ zIndex: 0 }}></div>

      {/* Layer 2: Background Image */}
      <div className="fixed top-16 left-0 right-0 h-[700px] pointer-events-none overflow-hidden" style={{ zIndex: 1 }}>
        <img src="/hero-bg.png" alt="" className="absolute inset-0 w-full h-full object-cover opacity-20" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8 text-sm font-medium">
              <Link to="/" className={`transition-colors flex items-center gap-1 ${location.pathname === '/' ? 'text-primary-400 font-bold' : 'text-slate-300 hover:text-primary-400'}`}>
              ...
              <Link to="/how-it-works" className={`transition-colors flex items-center gap-1 ${location.pathname === '/how-it-works' ? 'text-primary-400 font-bold' : 'text-slate-300 hover:text-primary-400'}`}>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-slate-900 border-b border-slate-800 absolute w-full left-0 top-16 shadow-2xl">
            <div className="flex flex-col px-6 py-6 space-y-5 font-medium">
              <Link to="/" className={`flex items-center gap-2 transition-colors ${location.pathname === '/' ? 'text-primary-400 font-bold' : 'text-slate-300 hover:text-primary-400'}`} onClick={closeMenu}>
              ...
              <Link to="/how-it-works" className={`flex items-center gap-2 transition-colors ${location.pathname === '/how-it-works' ? 'text-primary-400 font-bold' : 'text-slate-300 hover:text-primary-400'}`} onClick={closeMenu}>
      ...
      {/* Layer 3: Page Content (above backgrounds) */}
