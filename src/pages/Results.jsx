...
function Results() {
  ...
  const handleDownload = () => {
    const reportText = `
ETHICAL UX AUDIT REPORT - MAVERICK AUDIT
---------------------------------------
Website: ${scannedUrl}
Date: ${new Date().toLocaleString()}
Ethical Score: ${score}/100

DETECTED PATTERNS (${patterns.length}):
${patterns.map((p, i) => `
${i + 1}. ${p.type} (${p.severity})
   Description: ${p.description}
   Suggestion: ${p.suggestion}
`).join('\n')}

Generated by Maverick Audit Platform - Hacksagon 2026
    `.trim()
    ...
    link.href = url
    link.download = `audit-report-${scannedUrl.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.txt`
